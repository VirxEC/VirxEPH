...
from rlbot.utils.structures.game_data_struct import GameTickPacket, Vector3, PlayerInfo, Touch
...
class PacketHeuristics:
    def __init__(self, threshold: float=0.8, gain: float=0.21, loss: float=0.0045, unpause_delay: float=0.75, ignore_indexes: List[int]=[], verbose=False):
        self.cars = {}
        self.car_tracker = {}
...
class Zone2D:
    ...
    def intersect_sphere(self, l: np.ndarray, r: float) -> bool:
        nearest = np.array([
            cap(l[0], self.min[0], self.max[0]),
            cap(l[1], self.min[1], self.max[1]),
            cap(l[2], self.min[2], self.max[2])
        ])
