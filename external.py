    ...
    def main(self):
        while 1:
            try:
                ...
                self.renderer.draw_polyline_3d(tuple(ball_slice.physics.location for ball_slice in ball_prediction_struct.slices[::2]), self.renderer.yellow())
                ...
                for i in range(packet.num_cars):
                    ...
                    self.renderer.draw_string_3d(car.physics.location, 1, 1, f"Zone id: {zone_id}\n{surrounding_zone_ids}\n{raw_predictions}\n{true_predictions}", self.renderer.cyan())
            except Exception:
                print_exc()
            finally:
